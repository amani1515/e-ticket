...
<div class="p-6 bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen">
    ...
    <div class="bg-white rounded-xl shadow-lg">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody id="sync-table-body" class="bg-white divide-y divide-gray-200">
                    @forelse($syncData ?? $recentSync ?? [] as $item)
        ...
        <!-- Pagination -->
        @if(isset($syncData) && $syncData->hasPages())
        <div class="px-6 py-4 border-t border-gray-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    <span>Showing {{ $syncData->firstItem() }} to {{ $syncData->lastItem() }} of {{ $syncData->total() }} results</span>
                </div>
                <div class="flex items-center space-x-2">
                    {{ $syncData->links('pagination::tailwind') }}
                </div>
            </div>
        </div>
        @endif
