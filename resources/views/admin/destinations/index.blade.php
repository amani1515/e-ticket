...
<div class="min-h-screen bg-gradient-to-br from-yellow-50 to-amber-100 p-4 sm:p-8">
    <div class="max-w-7xl mx-auto">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-amber-900 mb-2">🗺️ Destinations Management</h1>
            <p class="text-amber-700">Manage all bus destinations and routes</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div class="flex gap-4">
                <div class="bg-white rounded-lg shadow px-4 py-2">
                    <div class="text-sm text-amber-600">Total Destinations</div>
                    <div class="text-2xl font-bold text-amber-900">{{ $destinations->total() }}</div>
                </div>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('admin.destinations.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-amber-700 mb-2">Search Destinations</label>
                        <div class="relative">
                            <input type="text" name="search" value="{{ request('search') }}" 
                                   placeholder="Search by destination name or start location..."
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.destinations.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                    ...
                    <table class="min-w-full">
                        <thead class="bg-gradient-to-r from-amber-600 to-yellow-600 text-white">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold">Destination</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold">Start From</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold">Distance</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold">Tariff</th>
                        ...
                        <tbody class="divide-y divide-amber-100">
                            ...
                            <tr class="hover:bg-amber-50 transition-colors duration-200">
                                <td class="px-6 py-4 text-sm text-amber-900 font-medium">
                                    {{ ($destinations->currentPage() - 1) * $destinations->perPage() + $index + 1 }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold text-amber-900">{{ $destination->destination_name }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-amber-700">{{ $destination->start_from }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                        {{ number_format($destination->tariff) }} ETB
                                    </span>
                                </td>
                                <td class="px-6 py-4">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-amber-900">No destinations found</h3>
                    <p class="mt-1 text-sm text-amber-600">Try adjusting your search criteria.</p>
                </div>
            @endif
        </div>
