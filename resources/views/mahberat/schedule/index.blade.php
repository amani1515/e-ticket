                        ...
                        <td class="px-6 py-4">
                            @if($schedule->status !== 'departed')
                                <div class="flex space-x-2">
                                    ...
                                    <form action="{{ route('mahberat.schedule.mark-departed', $schedule->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to mark this schedule as departed?');" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-green-600 hover:text-green-800 px-2 py-1 rounded transition" title="Mark as Departed">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
