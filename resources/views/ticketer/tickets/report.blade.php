...
@section('content')
    <div class="flex min-h-screen">
            ...
            <form method="GET" action="{{ route('ticketer.tickets.report') }}" class="mb-6 flex space-x-4">
                ...
                <div>
                    ...
                    <select name="ticket_status" id="ticket_status" class="w-full p-2 border rounded">
                        ...
                        <option value="created" {{ request('ticket_status') == 'created' ? 'selected' : '' }}>Created
                        </option>
                        <option value="confirmed" {{ request('ticket_status') == 'confirmed' ? 'selected' : '' }}>Confirmed
                        </option>
                    </select>
                </div>

                <div>
                    <label for="search" class="block text-sm font-medium text-gray-700">Search (Targa or Ticket
                        ID)</label>
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        class="w-full p-2 border rounded" placeholder="Enter Targa or Ticket ID">
                </div>
                ...
                <div>
                    ...
                    <select name="date_filter" id="date_filter" class="w-full p-2 border rounded">
                        ...
                        <option value="two_days_before"
                            {{ request('date_filter') == 'two_days_before' ? 'selected' : '' }}>2 Days Before</option>
                    </select>
                </div>
                ...
                <div class="flex items-end">
                    ...
                    <a href="{{ route('ticketer.tickets.export', request()->query()) }}"
                        class="bg-green-500 text-white px-4 py-2 rounded ml-2">Export to Excel</a>
                </div>
            </form>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="border px-4 py-2">
                                        @if ($ticket->ticket_status === 'created')
                                            <span
                                                class="bg-red-100 text-red-700 text-sm px-3 py-1 rounded-full">Created</span>
                                        @elseif($ticket->ticket_status === 'confirmed')
                                            <span
                                                class="bg-green-100 text-green-700 text-sm px-3 py-1 rounded-full">Confirmed</span>
                                        @else
                                            <span
                                                class="bg-gray-100 text-gray-700 text-sm px-3 py-1 rounded-full">{{ ucfirst($ticket->ticket_status) }}</span>
                                        @endif
                                    </td>
        ...
        '{{ $ticket->disability_status }}'
                                        ...
                                        @if ($ticket->ticket_status === 'created')
                                            <form action="{{ route('ticketer.tickets.cancel', $ticket->id) }}"
                                        ...
                                        <a href="{{ route('ticketer.tickets.receipt', $ticket->id) }}" target="_blank"
                                            class="bg-blue-500 text-white px-2 py-1 rounded text-sm ml-1">Print</a>
                        ...
                        <form id="editTicketForm" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" name="departure_datetime" id="departure_datetime"
                                    class="w-full p-2 border rounded" value="{{ now()->format('Y-m-d\TH:i') }}" required>
                            </div>
